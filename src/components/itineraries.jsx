...
const Itineraries = () => {
    ...
    const handleCityClick = (city) => {
        ...
        const y = carouselRef.current.getBoundingClientRect().top + window.scrollY + yOffset;
    ...
    return (
        <div>
            <Container fluid className="vh-100 d-flex flex-column justify-content-center align-items-center bg-dark">
            ...
            <div className="mt-3">
                {cities.map(city => (
                    <Button 
                        ...
                        className="mx-2"
                        style={{ backgroundColor: selectedCity === city.toLowerCase() ? "#88e097" : "#555", border: "none", color: "#000" }}
                    >
                        {city}
                    </Button>
                ))}
            ...
            <Container ref={carouselRef} className="my-4 p-3 rounded" style={{ backgroundColor: "#0b524c" }}>
                <Row>
                    <Col md={6}>
                        <h2 className="text-center p-3" style={{ color: "#F19EDC", fontWeight: "bold" }}>{selectedCity.toUpperCase()}</h2>
